...
class ZipPicViewHandler extends AbstractHandler {
	...
	def handleDirectory(String file, ZipFile zipFile, String innerFile, Map<String, String> queries, String target,
		...
		HttpServletResponse response) throws IOException, ServletException  {
			...
			val entries = Collections.list(zipFile.entries).filter [ ZipArchiveEntry entry |
			...
			for (entry : entries) {
				...
				if (entrySupported(entry)) {
					...
					jsonStringer.key("type").value(if(entry.isDirectory) "Directory" else "File")
		...
		def handleFile(String file, ZipFile zipFile, String innerFile, Map<String, String> queries, String target,
			Request baseRequest, HttpServletRequest request,
			HttpServletResponse response) throws IOException, ServletException {
				...
				if (thumbSize == null) {
					...
					val srcImg = read(inputStream)
					val destImg = Scalr.resize(srcImg, Scalr.Method.AUTOMATIC, Scalr.Mode.AUTOMATIC,
						Integer.parseInt(thumbSize))
					write(destImg, "jpg", response.outputStream)
				}
				inputStream.close
				baseRequest.handled = true
			}

			def entrySupported(ZipArchiveEntry entry) {
				val name = entry.name
